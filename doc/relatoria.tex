...
We consider make our own algorithm to solve pack but bucause it was not as efficient
as the algorithm proposed by [those dudes] so we opted to implement that algorithm.

\subsection{Split}
 The split pattern is a variant of pack which does not discard elements, but instead packs them to the top or bottom of the output collection.
