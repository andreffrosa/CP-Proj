...
efficient.
Our implementation makes use of several parallel patterns: in a first phase we use the fork-join pattern to compute
two scans, and then we use the map parallel pattern to sort the array as in the pack pattern.
...
In this Section, we will present some implementation details that we found relevant.

In the map pattern implementation we would like to point out that we left the the source array splitting
to the cilk runtime, for that cilk takes in consideration the size, number of the elements to process.
We tryed to make the batching separation ourselfs but, because we were getting pooer outcomes, and because
we couldn't make assumptions on data types we just left this task to cilk.
...
In the reduce pattern implementation, to avoid dataraces and keep the intermediates results contigous in memory,
we make use of two arrays, one to read from and another to write to, these arrays switch roles at each iteration of reduce.
