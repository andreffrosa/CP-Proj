...
The parallel computation paradigm has been emerging, as a consequence to the huge growth on the amount of data that needs to be processed and analyzed, and to switching of processors architecture from being smaller and clockwise fast to have parallel functionalities, such as more cores, hardware threads and vector operations. Therefore, it is imperative to build algorithms that explore these functionalities and allow to process such huge amounts of data.
However, making use of these functionalities to build algorithms can sometimes be a difficult and complex task, due to them being low level primitives and sometimes platform dependent. Thus, parallel functions and libraries can provide an easy and generic way to better utilize these resources, when available. Therefore, we developed a parallel patterns library, that implements some of the most well known parallel patterns, and that can be easily integrated in any already existing sequential program, to improve its efficiency. In order to be able to be used in a vast amount of use cases, these implementations are independent both from the data types they are manipulating and the parallel functionalities provided by each specific hardware platform.
