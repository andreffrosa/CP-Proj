...
\begin{abstract}
In recent years, the parallel computation paradigm has been emerging, as a consequence to: the huge growth on the amount of data that needs to be processed and analyzed, and to the switching of processors' architecture evolution process from becoming smaller and having faster clock speeds, to integrate parallel functionalities, such as multiple cores, hardware threads and vector operations. Therefore, it is imperative to build algorithms that explore these functionalities to increase the efficiency on processing such huge amounts of data.
However, building algorithms on top of these functionalities can sometimes be a difficult and complex task, due to them being low-level primitives and sometimes platform dependent. Thus, parallel functions and libraries can provide an easy and generic way to better utilize these resources, when available. Therefore, we developed a library, that implements some of the most well known parallel patterns, and that can be easily integrated with any already existing sequential program, to improve its efficiency.
...
\subsection{Map}
The Map pattern consists on the independent application of the same function (or worker) to every element of a collection. This function needs to be pure, e.g., not have side effects in order to be parallelizable. As the applications of the function are independent from one another, they can be executed in parallel.
...
\subsection{Pack}
The Pack pattern takes an collection and a binary mask, and returns the collection where only the elements of the input collection whose mask value was True (Without space between the elements).
...
We consider design our own algorithm to implement Pack, however, the version proposed in the classes was more efficient, and thus we opted to implement that instead. That version resorts to Scan to compute a the sum of the mask (bitsum) elements giving for each True element of the mask the corresponding position in the output collection. Then, a map operation is applied that for all the elements of the input collection that have True in the corresponding mask position, insert at the position given by the bitsum (for the current element) in the output collection. This implementation have work O(n) and span O(log n).
