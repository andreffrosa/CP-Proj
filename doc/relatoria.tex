...
The parallel version of the Gather pattern shows an improvement in terms of latency when compared to its sequential counterpart as seen in Figure~\ref{latency}. This is to be expected, since parallel Gather is implemented in a similar to to parallel Map, maximizing potential parallelism. Since Gather only performs reads, there is no danger of cache invalidation as with parallel Scatter, which means the cache memories are used to their fullest extent, increasing its performance. In fact, since Gather executes so quickly, we had trouble getting accurate execution time values for input sizes under 80000.

\subsubsection{Scan}
The parallel version of the Scan pattern prsents a worse performance compared to its sequential counterpart as seen in Figure~\ref{latency}. Our inmplementation of parallel Scan performs a very big memory allocation at the start of its execution of the prefix-sum algorithm, in order to perform static memory accesses, and decrease execution time. The amount of memory allocated increases linearly with the size of the input. What's happening here is that for large inputs, the allocated memory does not fit into memory and is stored in secondary storage (disk). This leads to an increase in page faults when the processor does not find the relevant allocated in memory. The parallel algorithm has an increase of the amount of page faults by a factor of 3 when compared to its sequential implementation, as observed by us with the perf tool.
