...
Nowadays, the parallel paradigm is a matter of great importance, and the demand for more scalabel and efficient data processing algorithms has increased. [dizer um pouco mais]
...
%Falar da lei de moore (?) e dizer que existem limitações ao poder de processamento sequencial.
On the other hand, due to the limitations of processors designed to run serial code such as [Meter links para a power wall, ILP wall e memory wall penso nao ser preciso explicar o que são se tiver links] a revolution happened in the processor's architecture design. Architectures switched to have more parallel functionalities, such as multiple cores and hardware threads as well as vector operations, that allow the simultaneously execution of processes and parallel manipulation of multiple data.
...
%[Falar de que existem padrões paralelos, como tá no livro, para dizer que podem servir como building blocs de algortimos e assim] [meter citação para %https://www.usenix.org/legacy/event/hotpar10/tech/full_papers/McCool.pdf]


On that account, we designed and implemented a library that contains some of the most well known parallel patterns. [meter citação anteriror]
This library provides an easy way to update already-built sequential programs to make use of the hardware parallel functionalities, allowing the application-level developer to integrate such patterns and give its application a boost of performance with little effort. We resorted to Intel's Cilk$^+$ to implement those patterns. [meter citação para os dreds] In order to be able to use it in a vast amount of use cases, these implementations are independent both from the data types they are manipulating and the parallel functionalities provided by each specific hardware platform. We developed each algorithm to try to achieve the maximum possible parallel slack and ... [mudar um bocado isto]
...
\subsection{Reduce}
Explicar brevemente o que é
We implemented 2 versions of reduce: (regular) reduce and tiled reduce.
Explicar a difenreça
...
\subsection{Pack}
Our parallel implementaion of pack [ref para pack] make use of the scan function to calculate a bitSum array of
the filter values. We do this to calculate the positions of the values marked as toKeep
(with filter value equal to 1) with span equal to log n [é isto?]. After this we excute a map over the filter array and for each position i if the value is positive (equal to 1). We copy the value at position i of the source array to the position bitSum[i]-1 of the destination array.
This algorithm was proposed by [nomes do dreds]. We consider make our own algorithm to solve pack but bucause it was not as efficient we opted to implement this version of pack.

[é isto??]


\subsection{Split}
Função extra(secalhar dizer isto lá em cima)
